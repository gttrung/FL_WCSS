...
from base import BaseModel
from util.helpers import initialize_weights, set_trainable
from itertools import chain
from model import model_resnet as resnet
...
class UNetResnet(BaseModel):
    def __init__(self, num_classes, in_channels=3, backbone='resnet50', pretrained=True, freeze_bn=False, freeze_backbone=False, **_):
        ...
        # decoder
        self.conv1 = nn.Conv2d(512, 192, kernel_size=3, stride=1, padding=1)
        self.upconv1 =  nn.ConvTranspose2d(192, 128, 4, 2, 1, bias=False)

        self.conv2 = nn.Conv2d(384, 128, kernel_size=3, stride=1, padding=1)
        self.upconv2 = nn.ConvTranspose2d(128, 96, 4, 2, 1, bias=False)

        self.conv3 = nn.Conv2d(224, 96, kernel_size=3, stride=1, padding=1)
...
class NestedUnet(BaseModel):
    def __init__(self, num_classes, in_channels=3, backbone='resnet50', pretrained=True, freeze_bn=False, freeze_backbone=False, **_):
        ...
        self.model = smp.UnetPlusPlus(encoder_name=backbone, encoder_weights='imagenet', in_channels=in_channels, classes=num_classes, activation=None)
